// ============================================================
// Ternak Device Changer v5.0 — Zygisk hook module
// ============================================================
#include <jni.h>
#include <unistd.h>
...
// Protocol ke companion (dishare dengan companion.cpp)
enum : uint8_t {
    CMD_CHECK_TARGET = 1,
    CMD_GET_IDENTITY = 2,
};

// Per-process identity map (populated di postAppSpecialize)
...
// ============================================================
// Hook 1: SystemProperties.native_get(String,String) → String
// Cover semua backing native prop dari Build.* dan getSerial()
// ============================================================
static jstring hook_native_get(JNIEnv* env, jclass, jstring j_key, jstring j_def) {
    ...
    // Fallback: baca native prop asli via libc
...
// ============================================================
// Hook 2: Direct static field set untuk Build.* dan Build.VERSION.*
// ============================================================
...
static void apply_build_hooks(JNIEnv* env) {
    ...
    if (build) {
        static const std::vector<std::pair<const char*, const char*>> str_fields = {
            ...
            {"TYPE",         "TYPE"},
            {"TAGS",         "TAGS"},
            {"SERIAL",       "SERIAL"},   // deprecated static field, still set
            {"RADIO",        "RADIO"},
        };
...
// Companion handler forward declaration — implementasi di companion.cpp
